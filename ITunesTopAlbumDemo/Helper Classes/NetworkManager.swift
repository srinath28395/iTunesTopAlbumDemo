...
class NetworkManager {
    ...
    let baseURl = "https://rss.itunes.apple.com/api/"
    ...
    enum NetworkError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .noInternet:
                return "Internet Connextion seems to be offline"
            }
        }
    }
    ...
    enum EndPoints {
        case topAlbums(Int = 20)
        
        func value() -> String {
            switch self {
            case .topAlbums(let albumCount):
                return "v1/us/itunes-music/top-albums/all/\(albumCount)/explicit.json"
            }
        }
    }
